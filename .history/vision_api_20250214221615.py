...
# Beispielhafte Zuordnung von Mülltonnen zu weiteren Details
muelltonnen_details = {
    "Trash bin": {"size": "50L", "cost": "€30"},
    "Garbage bin": {"size": "120L", "cost": "€50"},
    "Waste container": {"size": "240L", "cost": "€100"}
}
...
def zaehle_tonnen(image_path, api_key):
    ...
    for obj in objects:
        if any(term in obj["name"] for term in muelltonnen_begriffe):
            details = muelltonnen_details.get(obj["name"], {"size": "Unknown", "cost": "Unknown"})
            info = {
                "name": obj["name"],
                "size": details["size"],
                "cost": details["cost"]
