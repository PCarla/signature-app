...
def save_signature():
    try:
        ...
        print("Received data type:", type(data))  # Debugging-Log
        print("Received data keys:", data.keys() if data else "No data")  # Debugging-Log


        if not data:
            print("No data received in request body")  # Debugging-Log
            return jsonify({"error": "No JSON data received in request body"}), 400


        signature_data = data.get("signature")
        print("Signature data type:", type(signature_data))  # Debugging-Log
        print("Signature data length:", len(signature_data) if signature_data else 0)  # Debugging-Log
        ...
        try:
            ...
            print("Attempting to save to:", file_path)  # Debugging-Log
            with open(file_path, "wb") as f:
                f.write(decoded_signature)
            print("File saved successfully")  # Debugging-Log
            # Verify file was written
            if os.path.exists(file_path):
                print("File verification: exists, size:", os.path.getsize(file_path), "bytes")  # Debugging-Log
            else:
                print("File verification failed: file does not exist")  # Debugging-Log
                return jsonify({"error": "File could not be saved"}), 500
        except IOError as e:
            print("File write error:", str(e))  # Debugging-Log
